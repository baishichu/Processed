...
if __name__ == "__main__":
    crop_times = 2
    resize = 227
    # dir = "/media/ph/208B-304E/train/CASME/CASME2/CASME2_RAW_selected"  # CASME
    ...
    for video, frames in videos.items():
        ...
        save_folder = '/media/ph/208B-304E/projectVQA/MEGC2025VQA/Processed/trainData3/SAMM/{}/{}'.format(
        ...
        if not row.empty:
            ...
            offset = int(row['Offset Frame'].values[0])  # samm
            filename=str(row['Filename'].values[0])  # samm
            emo = str(row['Estimated Emotion'].values[0])  # samm
            au = str(row['Action Units'].values[0])  # samm
            print("++++++++++++++++",filename,emo,au)
            # apex = int(row['ApexFrame'].values[0])  # CASME
            # onset = int(row['OnsetFrame'].values[0])  # CASME
            print(f"ApexFrame: {apex}, OnsetFrame: {onset}")
            print(f"start: {onset - onset},end: {apex - onset}")
        ...
        print("phoffset",frames[offset-onset-1])

        phOnset = cv2.imread(frames[onset - onset+2])
        phApex = cv2.imread(frames[apex - onset-2])
        phoffset = cv2.imread(frames[offset-onset-1])
